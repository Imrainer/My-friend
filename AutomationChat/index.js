        ...
        if(type === "notify"){
            if(!messages[0].key.fromMe) {
                ...
                if(!messages[0].key.fromMe && !messages[0].key.participant ){
                    let sql_clients = 'SELECT id,nama,nomor_hp,industri FROM clients='+pesanMasuk+
                    await conn.query(sql_clients,async function (err, result)
                     {
                        if(err) throw err;
                        ...
                        if (row_jawaban > 0){
                            for(let i=0;i<row_jawaban;i++){
                                ...
                                let nama = result[i].nama;
                                let nomor_hp = result[i].nomor_hp;
                ...
                // }else{
                //     await sock.sendMessage(noWa, {text: "Izin Testing..."},{quoted: messages[0] });
                // }
			}		
		}		
    });
...
//    const sendMessage = async (req, res) =>{
    ...
    sock.sendMessage(number + "@s.whatsapp.net", {text: pesankirim})
    .then((result) => {
...
//send Whatsapp Message Blast

// app.post("/send-message-blast", async (req, res) =>{
//     const
// })
